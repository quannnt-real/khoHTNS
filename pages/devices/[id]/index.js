...
export default function DeviceDetail() {
  ...
  const handleTransfer = async () => {
    if (!transferUserId) {
      setActionError('Vui lòng chọn người dùng');
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.message || 'Lỗi khi chuyển thiết bị');
      }
      
      setShowTransferModal(false);
      fetchDevice();
      alert('Chuyển thiết bị thành công!');
    } catch (err) {
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      setActionError('Lỗi khi tải lên hình ảnh. Vui lòng thử lại.');
    }
  };
  ...
  return (
    <div>
            ...
            <div className="flex justify-between mb-4">
              <div>
                ...
                <p className="mt-1 text-gray-600">
                  Trạng thái: <span className={`font-medium ${device.status === 'available' ? 'text-green-600' : 'text-amber-600'}`}>
      ...
      {showTransferModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            <h3 className="text-lg font-medium mb-4">Chuyển thiết bị {device.name}</h3>
            ...
            <div className="mb-4">
              <p className="text-sm text-gray-600 mb-4">
                Hiện đang mượn bởi: <span className="font-medium">{device.borrower?.name}</span>
              </p>
              
              <label htmlFor="transferUserId" className="form-label">Chuyển đến</label>
              <select
                id="transferUserId"
